...
class TestUploadEndpoint:
    ...
    def test_rejects_invalid_algorithm(self, client):
    ...
    def test_upload_valid_file_returns_202(self, client):
...
class TestStatusEndpoint:
    def test_unknown_task_returns_404(self, client):
        response = client.get("/api/schedule/status/nonexistent-uuid")
        assert response.status_code == 404


class TestResultsEndpoint:
    def test_unknown_task_returns_404(self, client):
        response = client.get("/api/schedule/results/nonexistent-uuid")
        assert response.status_code == 404


class TestDownloadEndpoint:
    def test_missing_file_returns_404(self, client):
        response = client.get("/api/schedule/download/nonexistent_file.xlsx")
        assert response.status_code == 404
...
class TestHistoryEndpoint:
    ...
    def test_history_with_filters(self, client):
        ...
        response = client.get("/api/history?algorithm=GA&status=complete")
        assert response.status_code == 200
        data = response.json()
        assert data["items"] == []
